...
(function () {
  function init() {
    ...
    var savedGuests = [];               // guest emails (optional) added to BOTH invites; survives re-renders
    ...
    // ---- Optional guests: emails added to BOTH invites. Add-a-row UI; the
    // cleaned list is cached in savedGuests so a mid-flow re-render (slot taken,
    // back) keeps what the provider typed. Returns { el, collect }. ----
    var GUEST_MAX = 10;
    ...
    function renderCombinedConfirm() {
      ...
      // Optional guests — same list is attached to both invites below.
      var guestsSection = buildGuestsSection();
      card.appendChild(guestsSection.el);
      ...
      btn.addEventListener('click', function () {
        ...
        var gCol = guestsSection.collect();
        if (gCol.invalid) {
        ...
        postForm(WEBHOOK_BOOKING, bookingParams('filming', filmingMeta.event_type_uri,
            fSlot.start_time, fCol.answers, gCol.guests))
          .then(function (res) {
            if (res && res.ok) {
              filmingBooked = true;
              bookingLinks.filming = res;
              ...
              postForm(WEBHOOK_BOOKING, bookingParams('scripting', scriptingMeta.event_type_uri,
                  sSlot.start_time, sCol.answers, gCol.guests))
                .then(function (res2) {
                  if (res2 && res2.ok) {
